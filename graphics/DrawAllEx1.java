...
public class DrawAllEx1 extends MFrame implements ActionListener {
	...
	public DrawAllEx1() {
		super(600,300);
		System.out.println("메인클래스 생성자 실행");
		setTitle("선그리기");
		makeAWT();
		p1 = new Panel();
	...
	public void actionPerformed(ActionEvent e) {
		System.out.println("action이 일어났을 때 ");
	...
	class MCanvas extends Canvas {  // 내부클래스
		
		
		@Override
		public void paint(Graphics g) {
			System.out.println("paint - Override 들어옴");
			
			// 4개의 tf 필드값을 정수형으로 리턴
			int a[] = new int[4];
			for (int i = 0; i < a.length; i++) {
				a[i] = Integer.parseInt(tf[i].getText());
			}
			
			// color 체크박스 상태를 : 0(R), 1(G), 2(B)로 셋팅 
			switch(color) {
			...
			// mode  >  0: 선그리기  / 1 : 사각형 / 2 : 원그리기    && 채우기 여부 체크
			switch(mode) {
			case 0 : 
			...
			case 1: 
				if(fillcb.getState()) 		g.fillRect(a[0], a[1], a[2], a[3]);
				else 						    g.drawRect(a[0], a[1], a[2], a[3]);
				break;
				
			case 2: 
				if(fillcb.getState()) 		g.fillOval(a[0], a[1], a[2], a[3]);
				else 						    g.drawOval(a[0], a[1], a[2], a[3]);
				break;
			}
	...
	public void selectPanel(int mode) {
		System.out.println("selectPanel함수 들어옴");
		setTitle(sl[mode] + "그리기");
		
		switch(mode)	{
		case 0 : // 선을 선택했을때 만들어지는 애 ! 
